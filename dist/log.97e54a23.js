...
let dataSum = localStorage.listeLog;
let dataLog = localStorage.credeb;
...
inputCodeUn.addEventListener('change', (c)=>{
    codeUn1 = c.target.value;
    console.log(codeUn1);
});
inputCodeDue.addEventListener('change', (d)=>{
    codeDue1 = d.target.value;
    console.log(codeDue1);
});
...
button.addEventListener('click', ()=>{
    if (TransactionDate.value && TransactionObjet.value && inputCodeDue.value && inputCodeUn.value) {
        addLogDataToTable(trsDate, codeUn1, codeDue1, trsObjet, deb, cresh);
        ...
        localStorage.setItem('credeb', JSON.stringify(sumTime));
        id++;
        idSum++;
    } else alert('Veuiller remplir tous les champs');
